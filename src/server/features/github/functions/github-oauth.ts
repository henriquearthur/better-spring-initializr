...
export async function startGitHubOAuthFromBff(): Promise<StartGitHubOAuthResponse> {
  try {
    ...
    const oauthSession = await import('../infra/github-oauth-session.server')
...
function getGitHubOAuthConfig(): GitHubOAuthConfig {
  const callbackUrl = requireEnvironmentVariable('GITHUB_OAUTH_CALLBACK_URL')

  try {
    new URL(callbackUrl)
  } catch {
    throw new EnvironmentConfigurationError(
      'GITHUB_OAUTH_CALLBACK_URL must be a valid absolute URL.',
    )
  }

  return {
    clientId: requireEnvironmentVariable('GITHUB_CLIENT_ID'),
    clientSecret: requireEnvironmentVariable('GITHUB_CLIENT_SECRET'),
    callbackUrl,
  }
}
